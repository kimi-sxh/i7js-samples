...
public class Listing_15_13_ReadOutLoud extends GenericTest {
    ...
    public void manipulatePdf(String dest) throws IOException {
        ...
        PdfFont font = PdfFontFactory.createFont(StandardFonts.HELVETICA, PdfEncodings.CP1252, PdfFontFactory.EmbeddingStrategy.PREFER_NOT_EMBEDDED);
        ...
        tagPointer.addTag(PdfName.Div.getValue());

        tagPointer.addTag(PdfName.Span.getValue());
        canvas.beginText();
        ...
        tagPointer.moveToParent().addTag(PdfName.Span.getValue()).getProperties().setExpansion("Doctor");
        canvas.openTag(tagPointer.getTagReference());
        canvas.newlineShowText("Dr.");
        canvas.closeTag();

        tagPointer.moveToParent().addTag(PdfName.Span.getValue());
        canvas.openTag(tagPointer.getTagReference());
        canvas.showText(" Strangelove or: How I Learned to Stop Worrying and Love the Bomb.");
        canvas.closeTag();

        tagPointer.moveToParent().addTag(PdfName.Span.getValue()).getProperties().setExpansion("Eyes Wide Shut.");
        canvas.openTag(tagPointer.getTagReference());
        canvas.newlineShowText("EWS");
        canvas.closeTag();
        canvas.endText();

        tagPointer.moveToParent().addTag(PdfName.Span.getValue()).getProperties().setLanguage("en-us").setAlternateDescription("2001: A Space Odyssey");
        ImageData img = ImageDataFactory.create(RESOURCE);
        canvas.openTag(tagPointer.getTagReference());
        Rectangle rectangle = new Rectangle(36,640,100,100/img.getWidth()*img.getHeight());
        ...
        tagPointer.moveToParent().addTag(PdfName.Span.getValue()).getProperties().setActualText("Seven Samurai.");
        canvas.openTag(tagPointer.getTagReference());
